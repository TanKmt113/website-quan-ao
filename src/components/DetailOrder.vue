    ...
    >
      <div v-if="detailOrder" class="flex flex-col gap-3">
        <div class="flex items-center gap-2">
          ...
          <Tag :value="getStatus(detailOrder.status)"></Tag>
        </div>
        <span v-if="detailOrder.createdAt" class="font-normal"
          >Ngày tạo đơn: {{ format(detailOrder.createdAt, "dd/MM/yyyy") }}</span
        >
        <div class="border border-gray-300 p-3 rounded-lg">
          <strong class="text-lg">Sản phẩm</strong>
          <div
            v-for="(item, index) in detailOrder.items"
            :key="index"
              ...
              <div class="flex flex-col gap-2">
                ...
                <div class="flex flex-wrap items-center justify-between">
                  ...
                  <strong
                    >{{
                      detailOrder.coupon
                        ? `- ${formatPrice(detailOrder.coupon?.CouponValue)}`
                        : 0
                    }}đ</strong
                  >
                </div>
            ...
            <div class="border border-gray-300 p-3 rounded-lg">
              <div class="flex items-center justify-between">
                ...
                <Button
                  ...
                  size="small"
                  @click="confirmOrder(detailOrder._id)"
                ></Button>
              </div>
          ...
          <div class="col-span-3 flex flex-col gap-2">
            ...
            <div class="border border-gray-300 p-3 rounded-lg whitespace-normal">
              ...
              <div class="bg-gray-100 rounded-lg p-3 flex flex-col gap-2">
                ...
                <span
                  >Họ tên:
                  <strong>{{ detailOrder?.shippingAddress?.fullName }}</strong></span
                >
                <span
...
const confirmOrder = async (id) => {
  ...
  if (res) {
    ...
    proxy.$notify("E", "Xác nhận không thành công", toast);
  }
};
</script>
<style>
